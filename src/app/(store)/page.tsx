  ...
  return (
    <section className="w-full h-screen md:h-[800px] mx-auto flex justify-center bg-zinc-900 overflow-hidden">
      <div className="absolute inset-0 h-screen md:h-[800px]">
        {/* Woman in a cream cable-knit turtleneck sweater */}
        <Image
          ...
          <div className="flex items-center gap-x-4">
            <Link
              ...
              className={cn(
                ...
                "border border-white/70 text-white rounded-none px-8 py-3 text-xs tracking-widest uppercase md:px-14 md:py-5 hover:text-zinc-900 hover:bg-white hover:border-white transition-all",
              )}
            >
              Shop Collection
            </Link>
    ...
    <section className="flex justify-start items-center gap-x-10 overflow-auto py-12">
      {collections.map(({ node }) => (
        <Link href={`/collections/${node.slug}`} key={`collection_circle_${node.id}`}>
          <div className={cn("relative bg-secondary rounded-full flex justify-center items-center", "w-[280px] h-[280px]")}>
            <Image
              src={keytoUrl(node.featuredImage.key)}
              alt={node.featuredImage.alt}
              width={320}
              height={320}
              className={cn("object-center object-cover hover:scale-105 transition-all duration-500", "w-[240px] h-[240px]")}
            />
          </div>
          <p className="text-black text-center mt-3 font-semibold">{node.label}</p>
        </Link>
      ))}
    </section>
  );
...
function FeaturedProductsCards({ products }: FeaturedProductsCardsProps) {
  return (
    <section className="container mt-12">
      <h2 className="font-semibold text-2xl md:text-3xl mb-1 md:mb-3">New Arrivals</h2>
    ...
    <section className="relative lg:space-x-5 space-y-5 lg:space-y-0 grid grid-cols-1 lg:grid-cols-3 max-h-[840px]">
      ...
      <div className="relative col-span-2 w-full h-[840px]">
        ...
        <div className="bg-zinc-800/20 flex justify-center items-center flex-col absolute w-full h-full top-0 left-0 text-white">
          ...
          <Link className={cn(buttonVariants({ size: "lg" }), "text-xl py-8 px-10")} href="/collections/sweaters">
            Discover Now
          </Link>
        </div>
      </div>
  ...
  return (
    <section className="max-w-[1920px] mx-auto h-[620px] md:h-[580px] bg-[#FFF8EE] grid grid-cols-12 my-16">
      <div className="relative w-full h-[340px] md:h-[580px] col-span-12 md:col-span-8 overflow-hidden">
        {/* Close-up of soft folded knitwear / merino wool sweaters */}
        <Image
          src="https://images.unsplash.com/photo-1582657233878-e69b7f3fcc94?auto=format&fit=crop&q=80&w=1920"
      ...
      <div className="col-span-12 md:col-span-4 pb-6 md:py-20 px-6 md:px-16">
        <h2 className="text-xl md:text-3xl font-semibold mb-3">Slow Fashion. Real Craft.</h2>
