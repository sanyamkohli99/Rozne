...
type SizeChartRow = { [key: string]: string };

function CustomSizeChartTable({ headers, rows }: { headers: string[]; rows: string[][] }) {
  return (
...
export function SizeChartDialog({ sizeChart }: { sizeChart?: string | null }) {
  let customChart: { headers: string[]; rows: string[][] } | null = null;
  if (sizeChart) {
    try {
      const parsed = JSON.parse(sizeChart);
      if (parsed.headers && parsed.rows) customChart = parsed;
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl">
        ...
        {customChart ? (
          <CustomSizeChartTable headers={customChart.headers} rows={customChart.rows} />
        ) : (
          <Tabs defaultValue="knitwear" className="w-full">
            ...
            <TabsContent value="knitwear">
              <CustomSizeChartTable headers={knitwearCols} rows={knitwearSizeChart.map(r => [r.size, r.chest, r.waist, r.hip, r.length])} />
            ...
            <TabsContent value="hosiery">
              <CustomSizeChartTable headers={hosieryCols} rows={hosierySizeChart.map(r => [r.size, r.waist, r.hip, r.legLength, r.footSize])} />
              <div className="mt-4 p-3 bg-zinc-50 rounded text-xs text-muted-foreground space-y-1">
                <p>
